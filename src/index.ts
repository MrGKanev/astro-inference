  ...
  return {
    ...
    hooks: {
      ...
      'astro:build:done': async ({ dir, pages, logger }) => {
        ...
        let count = 0;
        const fullTxtPages: LlmsFullPage[] = [];
        ...
        for (const { pathname } of pages) {
          ...
          fullTxtPages.push({ url: sourceUrl, markdown });
          ...
          const footerText = options.customFooter ?? `Generated by astro-inference | ${siteUrl}/llms.txt`;
        ...
        // Concatenate every page's markdown into a single llms-full.txt
        ...
        const fullTxtOutPath = fileURLToPath(new URL(llmsFullTxtPath, dir));

        try {
          await writeFile(fullTxtOutPath, buildLlmsFullTxt(siteName, fullTxtPages, footer), 'utf-8');
          logger.info(`Generated ${llmsFullTxtPath}`);
        } catch (err) {
          logger.warn(`Failed to write ${fullTxtOutPath}: ${err instanceof Error ? err.message : err}`);
        }
      },
    },
  };
